...
class PokemonServiceTest {
  ...
  private val pokemonModelMapper: PokemonModelMapper =
      spyk(Mappers.getMapper(PokemonModelMapper::class.java))
  ...
  fun `when findAll random then return Pokemons`() {
    every { pokemonRepository.findByIds(any<List<Long>>()) } returns emptyList()

    assertNotNull(pokemonService.findAll(null, 0, 10, emptyList(), true))
  }

  @Test
  fun `when findAll then return Pokemons`() {
    every { pokemonRepository.findAll(any<PageRequest>()) } returns
        PageImpl(emptyList(), PageRequest.of(0, 10), 0)

    assertNotNull(pokemonService.findAll(null, 0, 10, emptyList(), false))
  }
  ...
  fun `when findAll with name then return Pokemons`() {
    ...
    assertNotNull(pokemonService.findAll("abra", 0, 10, emptyList(), false))
  }
